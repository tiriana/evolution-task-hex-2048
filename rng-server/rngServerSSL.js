...
class Server {
  ...
  #handler = async (req, res) => {
    ...
    if (!/\/\d+/.test(req.url))
      return this.#sendError(res, "wrong url", req.url);

    const radius = parseInt(req.url.slice(1));
    if (radius > 20)
      return this.#sendError(res, "Radius is too big, max is 20");
    ...
    try {
      ...
      data.forEach((d, index) =>
        ["x", "y", "z", "value"].forEach((f) => {
          ...
          else if (!isFinite(d[f]))
            throw createFieldError("isn't finite number");
          else if (f !== "value" && Math.abs(d[f]) > radius)
            throw createFieldError(`is outside the radius ${radius}`);
        })
      );
    } catch (e) {
      return e.message;
