...
const href =
  (urlArg && urlArg.replace("--url=", "")) || "http://localhost:3000/";
...
describe("Hex game launch", () => {
  ...
  describe("radius 2", () => {
    ...
    it.only("should render correct field with data-x, data-y, data-z, data-value with 0", async () => {
      ...
      expect(await readDOMField(page, radius)).toEqual(
    ...
    describe("adding", () => {
      it.each([
        ...
        server.changeHandler((_, field) =>
          field.length === 0 ? startPosition : []
        );
        ...
        const field = await readDOMField(page, radius);
        expect(field.filter(({ value }) => value > 0)).toEqual(
          expect.arrayContaining(expected)
        );
